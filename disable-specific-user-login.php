<?php
/**
 * Plugin Name:       Disable Specific User Login
 * Plugin URI:        https://wordpress.org/plugins/disable-specific-user-login/
 * Description:        Disable login for selected users. Manage the blocked users from the "Disabled specific user login" admin screen.
 * Version:           3.0.0
 ...
 * Text Domain:       disable-specific-user-login
 *
...
function dnal_block_user_login( $user, $username, $password ) {
	...
	if ( in_array( (int) $user->ID, dnal_get_blocked_ids(), true ) ) {
		return new WP_Error(
			...
			__( 'Login is currently disabled for this account.', 'disable-specific-user-login' )
		);
	}
...
function dnal_register_settings_page() {
	add_menu_page(
		__( 'Disabled specific user login', 'disable-specific-user-login' ),
		__( 'Disabled specific user login', 'disable-specific-user-login' ),
		'manage_options',
		'dnal-login-restrictions',
...
function dnal_enqueue_admin_assets( $hook ) {
	if ( 'toplevel_page_dnal-login-restrictions' !== $hook ) {
		return;
	}
...
function dnal_plugin_action_links( $links ) {
	$settings_link = sprintf(
		'<a href="%s">%s</a>',
		esc_url( admin_url( 'admin.php?page=dnal-login-restrictions' ) ),
		esc_html__( 'Settings', 'disable-specific-user-login' )
	);
...
function dnal_render_settings_page() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'You do not have permission to access this page.', 'disable-specific-user-login' ) );
	}
	...
	if ( isset( $_POST['dnal_save'] ) ) {
		...
		$notice = __( 'Login restrictions updated.', 'disable-specific-user-login' );
	}
	...
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Disabled specific user login', 'disable-specific-user-login' ); ?></h1>
		<p><?php esc_html_e( 'Check a user to disable their login. Unchecked users can log in normally.', 'disable-specific-user-login' ); ?></p>
		...
		<p>
			...
			<?php esc_html_e( 'user(s) currently blocked.', 'disable-specific-user-login' ); ?>
		</p>

		<form method="get" class="dnal-search-form">
			<input type="hidden" name="page" value="dnal-login-restrictions" />
			<label class="screen-reader-text" for="dnal-search-input"><?php esc_html_e( 'Search users', 'disable-specific-user-login' ); ?></label>
			<input type="search" id="dnal-search-input" name="dnal_search" value="<?php echo esc_attr( $search ); ?>" placeholder="<?php esc_attr_e( 'Search name, login or email', 'disable-specific-user-login' ); ?>" />
			<?php submit_button( __( 'Search', 'disable-specific-user-login' ), '', '', false ); ?>
		</form>
		...
		<form method="post">
			...
			<table class="widefat striped dnal-table">
				<thead>
					<tr>
						<td class="check-column"><?php esc_html_e( 'Block', 'disable-specific-user-login' ); ?></td>
						<th class="dnal-sort" data-type="num"><?php esc_html_e( 'ID', 'disable-specific-user-login' ); ?><span class="dnal-arrow"></span></th>
						<th class="dnal-sort" data-type="str"><?php esc_html_e( 'Name', 'disable-specific-user-login' ); ?><span class="dnal-arrow"></span></th>
						<th class="dnal-sort" data-type="str"><?php esc_html_e( 'Username', 'disable-specific-user-login' ); ?><span class="dnal-arrow"></span></th>
						<th class="dnal-sort" data-type="str"><?php esc_html_e( 'Email', 'disable-specific-user-login' ); ?><span class="dnal-arrow"></span></th>
						<th class="dnal-sort" data-type="str"><?php esc_html_e( 'Role', 'disable-specific-user-login' ); ?><span class="dnal-arrow"></span></th>
					</tr>
				</thead>
				<tbody>
					<?php if ( empty( $users ) ) : ?>
						<tr><td colspan="6"><?php esc_html_e( 'No users found.', 'disable-specific-user-login' ); ?></td></tr>
					<?php else : ?>
						<?php
						foreach ( $users as $u ) :
							$user_roles = array();
			...
			<?php submit_button( __( 'Save Changes', 'disable-specific-user-login' ), 'primary', 'dnal_save' ); ?>
		</form>
	</div>
	<?php
}
